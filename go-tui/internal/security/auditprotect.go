// auditprotect.go - NIST 800-53 AU-9: Protection of Audit Information
//
// Implements cryptographic protection, integrity verification, and
// secure archival of audit logs for DoD IL5 compliance.
//
...
// Default retention period for archived logs (7 years per DoD requirements)
...
// AuditProtector provides cryptographic protection for audit logs.
...
type AuditProtector struct {
	...
	keyManager *AuditHMACKeyManager
...
// NewAuditProtector creates a new audit protector.
...
func NewAuditProtector(auditLogPath string) (*AuditProtector, error) {
	...
	keyManager := NewAuditHMACKeyManager(dir)
...
func (p *AuditProtector) SignLogEntry(entry AuditEvent) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *AuditProtector) saveChainWithRetry() error {
	var lastErr error
...
// AU-5: Returns error if all retries fail.
func (p *AuditProtector) writeWitnessWithRetry(entry LogChainEntry) error {
	var lastErr error
	for attempt := 0; attempt < p.maxRetries; attempt++ {
		if attempt > 0 {
			// Exponential backoff: 100ms, 200ms, 400ms...
...
func (p *AuditProtector) saveChainLocked() error {
...
func (p *AuditProtector) VerifyLogIntegrity() (bool, []string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *AuditProtector) DetectTampering() (*TamperReport, error) {
...
func (p *AuditProtector) checkFilePermissions(report *TamperReport) {
...
func (p *AuditProtector) checkFileTimestamps(report *TamperReport) {
...
func (p *AuditProtector) ProtectLogs() error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *AuditProtector) GetLogHash() (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *AuditProtector) ArchiveLogs(retentionDays int) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	if archivedCount > 0 {
		// Log archival event
		AuditLogEvent("", "AUDIT_ARCHIVED", map[string]string{
...
func (p *AuditProtector) verifyArchive(srcPath, dstPath string) error {
...
func (p *AuditProtector) computeHash(data []byte) string {
...
func (p *AuditProtector) loadChain() error {
...
func (p *AuditProtector) saveChain() error {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.saveChainLocked()
}

// loadOrGenerateHMACKey is DEPRECATED - use AuditHMACKeyManager instead.
// This function is kept for backward compatibility during migration.
// SECURITY WARNING: This function auto-generates keys which violates AU-9.
// For NIST 800-53 AU-9 compliance, use AuditHMACKeyManager.LoadKey() which
// requires explicit key configuration and does not auto-generate keys.
//
// Deprecated: Use NewAuditHMACKeyManager(dir).LoadKey() instead.
func loadOrGenerateHMACKey(dir string) ([]byte, error) {
	// Delegate to the new key manager for proper multi-source loading
	keyManager := NewAuditHMACKeyManager(dir)
	key, _, err := keyManager.LoadKey()
	return key, err
}
...
func (p *AuditProtector) writeWitness(entry LogChainEntry) error {
...
func (p *AuditProtector) VerifyWitness() (bool, []string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *AuditProtector) SetStrictMode(strict bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.strictMode = strict
}
...
func (p *AuditProtector) IsStrictMode() bool {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.strictMode
}
...
func (p *AuditProtector) SetRetryConfig(maxRetries int, baseWait time.Duration) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if maxRetries > 0 {
...
func (p *AuditProtector) GetRetryConfig() (maxRetries int, baseWait time.Duration) {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *AuditProtector) RotateKey(resignEntries bool) (*RotationResult, error) {
	if p.keyManager == nil {
...
func (p *AuditProtector) GetKeyMetadata() *HMACKeyMetadata {
	if p.keyManager == nil {
		return nil
	}
...
// Returns nil if key manager is not initialized.
func (p *AuditProtector) GetKeyManager() *AuditHMACKeyManager {
	return p.keyManager
}

// Close cleans up AuditProtector resources and zeros sensitive key material.
// SECURITY: Zero key material to prevent memory disclosure
func (p *AuditProtector) Close() {
	p.mu.Lock()
	defer p.mu.Unlock()
	// Zero the HMAC key
	if p.hmacKey != nil {
...
var (
	globalProtector     *AuditProtector
...
// GlobalAuditProtector returns the global audit protector instance.
// SECURITY CRITICAL: Check IsHealthy() before relying on audit protection.
func GlobalAuditProtector() *AuditProtector {
	globalProtectorOnce.Do(func() {
		auditPath := DefaultAuditPath()
		var err error
		globalProtector, err = NewAuditProtector(auditPath)
		if err != nil {
...
// GlobalAuditProtectorHealthy returns true if the global protector initialized successfully.
// SECURITY: Callers should check this before relying on audit integrity protection.
func GlobalAuditProtectorHealthy() bool {
	GlobalAuditProtector() // Ensure initialized
	return globalProtectorInitErr == nil
}

// GlobalAuditProtectorError returns the initialization error, if any.
func GlobalAuditProtectorError() error {
	GlobalAuditProtector() // Ensure initialized
	return globalProtectorInitErr
}

// SetGlobalAuditProtector sets the global audit protector instance.
func SetGlobalAuditProtector(protector *AuditProtector) {
