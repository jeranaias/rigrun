...
// FailureCallback is called when audit logging fails.
...
// Logger provides thread-safe audit logging with secret redaction.
...
type Logger struct {
	...
	onFailure     FailureCallback // Called SYNCHRONOUSLY when logging fails
...
// NewLogger creates a new audit logger at the specified path.
...
func (l *Logger) handleFailureLocked(err error) (callback func(error), callbackErr error) {
...
func (l *Logger) checkCapacityLocked() error {
...
func (l *Logger) getCapacityLocked() (usedMB, totalMB int64, err error) {
...
func (l *Logger) LogQuery(sessionID, tier, query string, tokens int, cost float64, success bool) error {
	// Truncate query
...
func (l *Logger) LogQueryWithError(sessionID, tier, query string, tokens int, cost float64, errMsg string) error {
	// Truncate query
	truncatedQuery := truncateQuery(query, MaxQueryLength)
...
func (l *Logger) LogEvent(sessionID, eventType string, metadata map[string]string) error {
	event := Event{
		Timestamp: time.Now(),
		EventType: eventType,
		SessionID: sessionID,
...
func (l *Logger) redactLocked(input string) string {
	result := input
...
func (l *Logger) checkRotationLocked() error {
	if l.maxSize <= 0 {
		return nil
	}
...
func (l *Logger) SetOnFailure(callback FailureCallback) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) SetHaltOnFailure(halt bool) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) IsHaltOnFailure() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) HasAuditFailed() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) GetLastFailure() error {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) GetFailureCount() int {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) IsCircuitBreakerOpen() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) SetCircuitBreakerThreshold(threshold int) {
	l.mu.Lock()
	defer l.mu.Unlock()
	if threshold > 0 {
...
func (l *Logger) ResetAuditFailure() {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.auditFailed = false
	l.lastFailure = nil
...
func (l *Logger) SetCapacityThresholds(warningMB, criticalMB int64) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *Logger) CheckCapacity() (usedMB, totalMB int64, err error) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// DefaultPath returns the default audit log path (~/.rigrun/audit.log).
...
// This can be used without a Logger instance.
...
// GlobalLogger returns the global audit logger instance.
...
// SetGlobalLogger sets the global audit logger instance.
...
// InitGlobalLogger initializes the global audit logger with the given path.
func InitGlobalLogger(path string, enabled bool) error {
	globalLoggerMu.Lock()
	defer globalLoggerMu.Unlock()
...
// Log logs an event to the global audit logger.
func Log(event Event) error {
	return GlobalLogger().Log(event)
}

// LogQuery logs a query to the global audit logger.
func LogQuery(sessionID, tier, query string, tokens int, cost float64, success bool) error {
	return GlobalLogger().LogQuery(sessionID, tier, query, tokens, cost, success)
}

// LogEvent logs a generic event to the global audit logger.
func LogEvent(sessionID, eventType string, metadata map[string]string) error {
	return GlobalLogger().LogEvent(sessionID, eventType, metadata)
}
